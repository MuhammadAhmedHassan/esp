...
function ReportTable({
  ...
  if (dateViaHoursDataObj && dateViaDataLength === 0) { return <div>{t('Report.Exception.NoDataFound')}</div>; }
    ...
    <div className="reports-table-container">
      <table className="reports-table">
        <thead>
          <tr>
            <th />
            {isAllScheduleSelected && (<th className="no-border" colSpan={4} />)}
            {isAllShiftTypeSelected && <th className="no-border" />}
          ...
          <tr>
            <th className="">{t('Report.HoursWorked.Employee')}</th>
            {isAllScheduleSelected && (<th className="table-row-headers" colSpan={4}>{t('Report.HoursWorked.ScheduleName')}</th>)}
            <th className="table-row-headers">{t('Report.HoursWorked.ShiftLabel')}</th>
            {isAllShiftTypeSelected && (<th className="table-row-headers">{t('Report.HoursWorked.ShiftType')}</th>)}
        ...
        <tbody>
          <tr>
            <th className="simple-headers" rowSpan={Object.keys(filteredDataByShiftId).length + 1}>
              {userName}
            </th>
          </tr>
            ...
            return (
              <tr key={`${shiftId}-${idx + 1}`}>
                ...
                {fromDateToDateArr.map((date) => {
                  ...
                  const calTime = document ? document.calculatedTimeInHMFormate : '';
                  return (
                    <td key={date} className="text-center">{calTime}</td>
                  );
                })}
              </tr>
            );
          ...
          <tr>
            ...
            <td className="no-border" />
            <td>{t('Report.HoursWorked.TotalCalculatedHours')}</td>
            {fromDateToDateArr.map((date, idx) => {
