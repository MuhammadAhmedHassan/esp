...
class MyTeamShiftHistory extends React.Component {
  constructor(props) {
    super(props);
    ...
    this.state = {
      ...
      toDate: new Date(),
      employeeShift: [{ id: '0', firstName: 'All', lastName: '' }],
      divisionId: 0,
      businessUnitId: 0,
      departmentId: 0,
      teamId: 0,
  ...
  employeeName = () => {
    ...
    fetch(`${Api.manageEmp.employeeName}`, {
      ...
      .then((response) => {
        if (response.statusCode === 200) {
          this.setState({ employeeShift: [{ id: '0', firstName: 'All', lastName: '' }].concat(response.data), employeeId: '0' });
        } else if (response.statusCode === 401) {
          const refreshToken = userService.getRefreshToken();
          refreshToken.then(() => {
            const tokens = userService.getToken();
  ...
  getExcelDownload = () => {
    const {
      token, modelMessage, userId, fromDate, toDate, scheduleId, shiftId, employeeId,
    } = this.state;
  ...
  getEmployees = () => {
    ...
    const {
      ...
      managerId, countryId, stateId, totalRecords, fromDate, toDate, employeeId,
      pageIndex, pageSize, organisationId, city, userId, shiftId, scheduleId,
    } = this.state;
    if (fromDate !== null && toDate !== null) {
      const data = {
        ...
        divisionId: parseInt(divisionId, 0),
  ...
  render() {
    const {
      loading, employees, fromDate, toDate, employee, employeeId, employeeShift, selectedYear,
      fromDateIsGreater, toDateIsSmaller, scheduleId, schedules, shiftId, shifts, year,
      pageIndex, pageSize, totalRecords, initialize, fileDownload, isFromDate, isToDate,
    } = this.state;
    const { t } = this.props;
    let counter = 1;
    const date = new Date().getFullYear();
    const isEnabled = scheduleId > 0 || shiftId > 0 || employeeId > 0;
    return (
      <>
        <div className="mb-4 coverage">
          <Form>
            <Row className="reloadBtn">
              <button type="button" className="arrowBtn mx-2" aria-hidden onClick={() => this.getExcelDownload()}><img className="pointer" src={DownloadIcon} alt="download icon" /></button>
            </Row>
            <Row>
              <Col lg={3}>
                <Form.Label>
                  {t('ShiftTemplatePage.employee')}
                </Form.Label>
                <select className="form-control dropHeight" name="employeeId" value={employeeId} onChange={this.handleChange}>
                  {
                      employeeShift.map(employee => (
            ...
            <tbody>
              {employees && employees.length > 0 && employees.map(data => (
                <tr>
                  ...
                  <td>
                    <div style={{ backgroundColor: `${data.exceptionColourCode}` }} className="employeeCode">
                      {data.exceptionType}
                    </div>
                  </td>
                  <td>
...
export default (withTranslation()(MyTeamShiftHistory));
