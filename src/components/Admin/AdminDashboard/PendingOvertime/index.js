...
class PendingOvertime extends Component {
  constructor(props) {
    super(props);
    let isAdmin = false;
      ...
      .then((response) => {
        if (response.statusCode === 200) {
          ...
          refreshToken.then(() => {
            ...
            this.setState({ token: tokens }, () => this.getPendingOvertime());
          });
        }
      })
      .catch((err) => {
  ...
  view =(managerId) => {
    ...
    if (isAdmin) {
      history.push({
        pathname: '/view-overtime',
        state: { id: managerId },
      });
    } else {
      history.push({
        pathname: '/overtime-manager',
        state: {
          status: 10,
        },
      });
  ...
  render() {
    const { t } = this.props;
    const {
      pendingOvertime, pageSize, loaded, totalRecords, pageIndex, isAdmin,
    } = this.state;
    return (
      <>
        <Container fluid>
                          ...
                          <Table responsive striped bordered>
                            <thead>
                              <tr className="table-header ">
                                <th className="pl-5">{t('Dashboard.PendingOvertime.ManagerName')}</th>
                                <th className="pl-5">{t('Dashboard.PendingOvertime.No')}</th>
                              </tr>
                            </thead>
                            <tbody>
                              {pendingOvertime.map(data => (
...
export default withTranslation()(PendingOvertime);
