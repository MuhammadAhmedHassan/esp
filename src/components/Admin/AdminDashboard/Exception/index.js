...
import './style.scss';
import {
  Row, Table, Col, Container, Button,
...
class Exception extends Component {
      ...
      .then((response) => {
        if (response.statusCode === 200) {
          ...
          refreshToken.then(() => {
            ...
            this.setState({ token: tokens }, () => this.getExceptions());
          });
        }
      })
      .catch((err) => {
  ...
  render() {
    const { t } = this.props;
    const {
      exceptions, pageSize, loaded, totalRecords, pageIndex, isAdmin,
    } = this.state;
    return (
      <>
            ...
            <Col>
              ...
              {!loaded ? (
                <>
                  <div>
                    <Row>
                       <Filter showEmployee="true" isAdmin={isAdmin} clearFilter={this.clearFilter} search={this.search} handleChange={this.handleChange} />
                    </Row>
                          ...
                          <Table responsive striped bordered>
                            <thead>
                              <tr className="table-header ">
                                <th className="pl-5">{t('Dashboard.Exception.UserName')}</th>
                                <th className="pl-5">{t('Dashboard.Exception.NoOfUsers')}</th>
                              </tr>
                            </thead>
                            <tbody>
                              {exceptions.map(data => (
...
export default withTranslation()(Exception);
