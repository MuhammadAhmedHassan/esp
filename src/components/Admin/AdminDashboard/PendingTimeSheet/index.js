...
class PendingTimeSheet extends Component {
    ...
    fetch(`${Api.dashboard.getDashboardDetails}`, {
      ...
      .then((response) => {
        if (response.statusCode === 200) {
          this.setState({
            pendingTimeSheet: response.data,
            loaded: false,
          ...
          refreshToken.then(() => {
            ...
            this.setState({ token: tokens }, () => this.getPendingTimeSheet());
          });
        }
      })
      .catch((err) => {
  ...
  clearFilter = () => {
   ...
   render() {
     const { t } = this.props;
     const {
       pendingTimeSheet, pageSize, loaded, totalRecords, pageIndex, isAdmin,
     } = this.state;
     return (
       <>
         <Container fluid>
                         ...
                         <Table responsive striped bordered>
                           <thead>
                             <tr className="table-header ">
                               <th className="pl-5">{t('Dashboard.PendingTimeSheet.ManagerName')}</th>
                               <th className="pl-5">{t('Dashboard.PendingTimeSheet.No')}</th>
                             </tr>
                           </thead>
                           <tbody>
                             {pendingTimeSheet.map(data => (
...
export default withTranslation()(PendingTimeSheet);
