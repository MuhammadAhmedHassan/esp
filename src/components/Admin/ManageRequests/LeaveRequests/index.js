/* eslint-disable radix */
/* eslint-disable react/destructuring-assignment */
/* eslint-disable import/no-unresolved */
/* eslint-disable jsx-a11y/label-has-associated-control */
/* eslint-disable react/prefer-stateless-function */
import React from 'react';
import { withTranslation } from 'react-i18next';
import { Link } from 'react-router-dom';
import {
  Button, ButtonGroup, Accordion, Card, Row, Col, Modal, Form,
} from 'react-bootstrap';
import { commonService } from '../../../../services/common.service';
import 'react-datepicker/dist/react-datepicker.css';
import DatePicker from 'react-multi-date-picker';
import DatePanel from 'react-multi-date-picker/plugins/date_panel';
import '../style.scss';
import Api from '../../../common/Api';
import { userService } from '../../../../services';
import PaginationAndPageNumber from '../../../shared/Pagination/index';

const constActiveTransactionType = 10;
const constCloseTransactionType = 0;
const constLeaveApprovedStatus = 20;
const constLeaveRejectStatus = 40;

class LeaveRequests extends React.Component {
  constructor(props) {
    super(props);
    const userId = userService.getUserId();
    const token = userService.getToken();
    const employeeId = props.location && props.location.state ? props.location.state.employeeId : 0;
    const isShowOverdue = props.location && props.location.state && props.location.state.employeeId;

    this.state = {
      token: `token ${token}`,
      userId,
      appliedLeaves: [],
      transactionType: constActiveTransactionType,
      openApprovedAndRejectModal: false,
      regionMessage: '',
      leaveId: 0,
      leaveStatus: 0,
      dateRange: '',
      employeesUnderManager: [],
      reportingEmployee: employeeId,
      pageIndex: 1,
      pageSize: 5,
      totalRecords: 0,
      modelUpdate: false,
      errorMessage: '',
      isShowOverdue,
    };
    this.handleChange = this.handleChange.bind(this);
    this.handleSwitchChange = this.handleSwitchChange.bind(this);
    this.closeApprovedAndRejectPopup = this.closeApprovedAndRejectPopup.bind(this);
    this.submitRequest = this.submitRequest.bind(this);
    this.closeCalendar = this.closeCalendar.bind(this);
  }

  componentDidMount() {
    // this.getAppliedLeaves(this.state.transactionType);
    this.getManagerEmployee();
  }


  componentDidUpdate() {
    const { loaded } = this.state;
    if (!loaded) {
      this.getAppliedLeaves(this.state.transactionType);
    }
  }

  updatePageNum = (pageNum) => {
    if (pageNum > 0) {
      this.setState({
        pageIndex: pageNum,
        loaded: false,
      });
    }
  }

  updatePageCount = (pageCount) => {
    this.setState({
      pageSize: pageCount,
      pageIndex: 1,
      loaded: false,
    });
  }

  handleClose = () => {
    this.setState({
      modelUpdate: false,
    });
  };


  getManagerEmployee = () => {
    const {
      token, userId, pageIndex,
      pageSize,
    } = this.state;

    // Get leave type
    fetch(`${Api.vacationManagement.getTeamMateList}`, {
      method: 'POST',
      headers: new Headers({
        token: `${token}`,
        Accept: 'application/json',
        'Content-Type': 'application/json',
      }),
      body: JSON.stringify({
        languageId: 1,
        userId,
        pageIndex,
        pageSize,
      }),
    })
      .then(response => response.json())
      .then((response) => {
        if (response.statusCode === 200) {
          this.setState({
            employeesUnderManager: response.data,
            // pageIndex: response.data.pageIndex || 1,
            // pageSize: response.data.pageSize,
            // totalRecords: response.data.totalRecords,
          });
        } else if (response.statusCode === 401) {
          const refreshToken = userService.getRefreshToken();
          refreshToken.then(() => {
            const tokens = userService.getToken();
            this.setState({ token: tokens }, () => this.getManagerEmployee());
          });
        }
      })
      .catch(err => console.error(err.toString()));
  };

  getAppliedLeaves = (transactionType) => {
    const {
      token, userId,
      reportingEmployee, pageIndex,
      pageSize,
    } = this.state;
    let startDate = null;
    let endDate = null;
    if (this.state.dateRange != null && this.state.dateRange !== '') {
      if (this.state.dateRange.length > 0) {
        startDate = this.state.dateRange[0].toDate();
        endDate = this.state.dateRange.length > 1 ? this.state.dateRange[1].toDate() : null;
      }
    }

    fetch(`${Api.vacationManagement.getuserappliedleaves}`, {
      method: 'POST',
      headers: new Headers({
        token: `${token}`,
        Accept: 'application/json',
        'Content-Type': 'application/json',
      }),
      body: JSON.stringify({
        employeeId: parseInt(reportingEmployee, 10),
        appliedToSecondaryUser: userId,
        pageIndex,
        pageSize,
        transactionType: parseInt(transactionType),
        isClosed: transactionType !== constActiveTransactionType,
        fromDateTimeUtc: startDate,
        toDateTimeUtc: endDate,
      }),
    }).then(response => response.json())
      .then((response) => {
        // eslint-disable-next-line no-console
        if (response.statusCode === 200) {
          this.setState({
            appliedLeaves: response.data.userAppliedLeaves,
            openApprovedAndRejectModal: false,
            pageIndex: response.data.pageIndex || 1,
            pageSize: response.data.pageSize,
            totalRecords: response.data.totalRecords,
            loaded: true,
          });
        } else if (response.statusCode === 401) {
          const refreshToken = userService.getRefreshToken();
          refreshToken.then(() => {
            const tokens = userService.getToken();
            this.setState({ token: tokens }, () => this.getAppliedLeaves(transactionType));
          });
        }
      })
      .catch(err => console.error(err.toString()));
  }

  // Handler for Select
  handleDropDown = (event) => {
    this.setState(
      {
        [event.target.name]: event.target.value,
      }, () => this.getAppliedLeaves(this.state.transactionType),
    );
  };


  handleChange(event) {
    const { target } = event;
    const { name } = target;
    if (name === 'regionMessage') {
      this.setState({ [name]: target.value });
    }
  }

  handleSwitchChange(status) {
    this.setState({ transactionType: status });
    this.getAppliedLeaves(status);
  }

  handleApprovedAndReject(id, status, approval) {
    if (approval) {
      this.setState({ leaveId: id, leaveStatus: status }, () => this.submitRequest());
    } else {
      this.setState({
        openApprovedAndRejectModal: true, leaveId: id, leaveStatus: status,
      });
    }
  }

  closeApprovedAndRejectPopup() {
    this.setState({ openApprovedAndRejectModal: false, requestError: false });
  }

  closeCalendar() {
    this.getAppliedLeaves(this.state.transactionType);
  }

  submitRequest() {
    if ((this.state.regionMessage == null || this.state.regionMessage === '')
      && this.state.leaveStatus === constLeaveRejectStatus) {
      this.setState({
        requestError: 'Please enter the note/reason to reject the leave.',
      });
      return;
    }
    this.setState({
      requestError: '',
    });


    const {
      token, userId, leaveId, modelUpdate, errorMessage,
    } = this.state;
    fetch(`${Api.vacationManagement.leaveRejectedApproved}`, {
      method: 'POST',
      headers: new Headers({
        token: `${token}`,
        Accept: 'application/json',
        'Content-Type': 'application/json',
      }),
      body: JSON.stringify({
        id: this.state.leaveId,
        status: this.state.leaveStatus,
        approvedByUserId: userId,
        approverComments: this.state.regionMessage,
        languageId: 1,
      }),
    }).then(response => response.json())
      .then((response) => {
        // eslint-disable-next-line no-console
        if (response.statusCode === 200) {
          this.setState({
            openApprovedAndRejectModal: false,
            modelUpdate: true,
            errorMessage: response.message,
          });
          this.getAppliedLeaves(constActiveTransactionType);
        } else {
          this.setState({
            modelUpdate: false,
            errorMessage: response.errors,
          });
        }
      })
      .catch(err => console.error(err.toString()));
  }

  renderleavelist() {
    const { t } = this.props;
    const { isShowOverdue } = this.state;
    return (
      (
        this.state.appliedLeaves.map(appliedLeavesData => (
          <div className="appliedLeaveOuterCard">
            <div className="appliedLeaveCard">
              <Row>
                <Col md={4}>
                  <h5> Vacation Category</h5>
                  <p>
                    {appliedLeavesData.parentLeaveTypeName}
                  </p>
                </Col>
                <Col md={4}>
                  <h5> Vacation Type </h5>
                  <p>
                    {appliedLeavesData.childLeaveTypeName}
                  </p>
                </Col>
                <Col md={4}>
                  <h5> No of Days </h5>
                  <p>
                    {appliedLeavesData.noOfDays}
                  </p>
                </Col>
              </Row>

              <Row>
                <Col md={4}>
                  <h5> Start Date </h5>
                  <p>
                    {appliedLeavesData.strFromDateTimeUtc && commonService.localizedDate(appliedLeavesData.strFromDateTimeUtc)}
                  </p>
                </Col>
                <Col md={4}>
                  <h5> End Date </h5>
                  <p>
                    {appliedLeavesData.StrToDateTimeUtc && commonService.localizedDate(appliedLeavesData.StrToDateTimeUtc)}
                  </p>
                </Col>
                <Col md={4}>
                  <h5> Applied by</h5>
                  <p>
                    {' '}
                    {appliedLeavesData.appliedByUser}
                    {' '}
                  </p>
                </Col>
              </Row>
              {isShowOverdue && (
                <Row>
                  <Col md={4}>
                    <h5>{t('ManageRequest.ExceptionRequests.ChangeTime.OverDue')}</h5>
                    <p>
                      {' '}
                      {appliedLeavesData.overDueApprovalDetails}
                      {' '}
                    </p>
                  </Col>
                </Row>
              )}
              <Col lg={12} className=" d-flex align-items-center justify-content-end mb-3">
                <Link
                  className="btn btn-primary"
                  to={`/vacation-management/my-vacation/applied-vacation/view-details/${appliedLeavesData.id},${appliedLeavesData.parentLeaveTypeId},${appliedLeavesData.userId}`}
                >
                  View Details
                </Link>
              </Col>
              
              {this.state.transactionType === constActiveTransactionType
                ? (
                  <>
                    <hr />
                    <div className="row text-lg-right mt-4">
                      <div className="col-md-12">
                        <button type="button" className="btn btn-danger" onClick={() => this.handleApprovedAndReject(appliedLeavesData.id, constLeaveRejectStatus, false)}>Reject</button>
                        <button type="button" className="btn btn-success" onClick={() => this.handleApprovedAndReject(appliedLeavesData.id, constLeaveApprovedStatus, true)}>Approve</button>
                      </div>
                    </div>
                  </>
                ) : null
              }

            </div>
          </div>

        ))
      )

    );
  }

  render() {
    const {
      reportingEmployee, employeesUnderManager, regionMessage, requestError,
      pageIndex, pageSize, totalRecords, modelUpdate, errorMessage,
    } = this.state;

    const { t } = this.props;
    return (
      <div className="container-fluid">
        <div className="row">
          <div className="col-md-12">
            <div className="card_layout leaveManagement manageLeave">
              <h2>
                {t('VacationRequest.leaveRequest')}
                {' '}
              </h2>
              <div className="row ">
                <div className="col-lg-5">
                  <label className="form-check-label customCheck mb-1"> &nbsp; </label>
                  <ButtonGroup aria-label="Basic example">
                    <Button type="Button" variant="secondary" className={this.state.transactionType == constActiveTransactionType ? 'Active' : 'closed'} onClick={() => this.handleSwitchChange(constActiveTransactionType)}>{t('VacationRequest.activeBtn')}</Button>
                    <Button variant="secondary" className={this.state.transactionType != constActiveTransactionType ? 'Active' : 'closed'} onClick={() => this.handleSwitchChange(constCloseTransactionType)}>{t('VacationRequest.closedBtn')}</Button>
                  </ButtonGroup>
                </div>
                <div className="col-lg-1">  </div>
                <div className="col-lg-6">
                  <div className="row">
                    <div className="col-lg-6">
                      <label className="form-check-label customCheck mb-2">
                        {' '}
                        {t('VacationRequest.selectDateRange')}
                        {' '}
                      </label>
                      <DatePicker
                        range
                        onClose={() => this.closeCalendar()}
                        onChange={(array) => { // Array of Dateobjecs
                          this.setState({ dateRange: array });
                        }}
                        plugins={[
                          <DatePanel />,
                        ]}
                      />
                    </div>
                    <div className="col-lg-6">
                      <label className="form-label" htmlFor={reportingEmployee}>{t('VacationRequest.selectEmployee')}</label>
                      <select
                        className="form-control"
                        name="reportingEmployee"
                        value={reportingEmployee}
                        onChange={this.handleDropDown}
                      >
                        <option value="0">{t('VacationRequest.searchText')}</option>
                        {
                          employeesUnderManager.map(usr => (
                            <option key={usr.id} value={usr.id}>
                              {usr.name}
                            </option>
                          ))}
                      </select>
                    </div>

                  </div>
                </div>

              </div>
              {this.renderleavelist(this.state.userList)}
              <br />
              <PaginationAndPageNumber
                totalPageCount={Math.ceil(totalRecords / pageSize)}
                totalElementCount={totalRecords}
                updatePageNum={this.updatePageNum}
                updatePageCount={this.updatePageCount}
                currentPageNum={pageIndex}
                recordPerPage={pageSize}
              />
            </div>
          </div>

        </div>
        
        <Modal
          show={this.state.openApprovedAndRejectModal}
          onClose={this.closeApprovedAndRejectPopup}
          backdrop="static"
          keyboard={false}
        >
          <Modal.Header closeButton>
            <Modal.Title />
            <Modal.Body className="p-0">
              <Form.Group className="mb-3" controlId="formBasicEmail">
                <Form.Label>Notes</Form.Label>
                <Form.Control
                  type="text"
                  onChange={this.handleChange}
                  name="regionMessage"
                  id="regionMessage"
                  placeholder="Add Detail"
                  maxLength="256"
                />
                {
                  requestError && !regionMessage && <span className="error mandatory text-left">{requestError}</span>
                }
              </Form.Group>
            </Modal.Body>
          </Modal.Header>
          <Modal.Footer className="text-center">
            <Button variant="primary" onClick={() => this.submitRequest()}>
              Ok
            </Button>
            <Button variant="secondary" onClick={() => this.closeApprovedAndRejectPopup()}>
              Cancel
            </Button>
          </Modal.Footer>
        </Modal>
       
        <Modal
          show={modelUpdate}
          onHide={this.handleClose}
          backdrop="static"
          keyboard={false}
        >
          <Modal.Header closeButton>
            <Modal.Title>
              <p>
                {errorMessage }
              </p>
            </Modal.Title>
          </Modal.Header>
          <Modal.Footer>
            <Button variant="secondary" onClick={() => this.handleClose()}>
              Ok
            </Button>
          </Modal.Footer>
        </Modal>
      </div>
    );
  }
}

export default withTranslation()(LeaveRequests);
